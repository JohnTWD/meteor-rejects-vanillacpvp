...
/*
    ...
    Also I stole this exact code from https://github.com/etianl/Trouser-Streak/blob/main/src/main/java/pwn/noobs/trouserstreak/modules/BaseFinder.java#L54
*/
public class BaseFinderNew extends Module {
    ...
    public BaseFinderNew() {
        super(MeteorRejectsAddon.CATEGORY, "BaseFinderNew", "Estimates if a build or base may be in the chunk based on the blocks it contains.");
    }
    ...
    private void onReadPacket(PacketEvent.Receive event) {
        if (!(event.packet instanceof PlayerMoveC2SPacket) && event.packet instanceof ChunkDataS2CPacket && mc.world != null) {
            ...
            if (mc.world.getChunkManager().getChunk(packet.getX(), packet.getZ()) == null) {
                ...
                if (!Blawcks1.get().isEmpty() || !Blawcks2.get().isEmpty() || !Blawcks3.get().isEmpty() || !Blawcks4.get().isEmpty() || !Blawcks5.get().isEmpty() || !Blawcks6.get().isEmpty() || !Blawcks7.get().isEmpty()) {
                    try {
                        for (int x = 0; x < 16; x++) {
                                        ...
                                        if (!(blerks.getBlock() == Blocks.STONE)) {
                                            if (!(blerks.getBlock() == Blocks.DEEPSLATE) && !(blerks.getBlock() == Blocks.DIRT) && !(blerks.getBlock() == Blocks.GRASS_BLOCK) && !(blerks.getBlock() == Blocks.WATER) && !(blerks.getBlock() == Blocks.SAND) && !(blerks.getBlock() == Blocks.GRAVEL) && !(blerks.getBlock() == Blocks.BEDROCK) && !(blerks.getBlock() == Blocks.NETHERRACK) && !(blerks.getBlock() == Blocks.LAVA)) {
                                                if (spawner.get()) {
                                                    ...
                                                    if (mc.world.getRegistryKey() == World.OVERWORLD && (blerks.getBlock() == Blocks.MOSSY_COBBLESTONE || blerks.getBlock() == Blocks.COBWEB || blerks.getBlock() == Blocks.STONE_BRICK_STAIRS || blerks.getBlock() == Blocks.BUDDING_AMETHYST))
                                                        spawnernaturalblocks = true;
                                                    else if (mc.world.getRegistryKey() == World.NETHER && (blerks.getBlock() == Blocks.NETHER_BRICK_FENCE || blerks.getBlock() == Blocks.CHAIN))
                                                        spawnernaturalblocks = true;
                                                }
