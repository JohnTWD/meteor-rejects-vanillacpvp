...
// stolen from https://github.com/RickyTheRacc/banana-for-everyone/blob/main/src/main/java/bananaplus/modules/combat/CevBreaker.java#L35
...
public class CevBreaker extends Module {
    ...
    public CevBreaker() {
        super(MeteorRejectsAddon.CATEGORY, "cev-breaker", "why tf is it called cevbreaker");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if ((mc.player.getInventory().selectedSlot == pickSlot || switchDelayLeft <= 0) && crystalThere && blockState.isOf(Blocks.OBSIDIAN)) {
            Direction direction = RejectEntityUtils.rayTraceCheck(blockPos, true);
            if (mode.get() == Mode.Instant) {
                ...
                timer = startedYet ? timer : RejectEntityUtils.getBlockBreakingSpeed(blockState, blockPos, pickSlot);
                if (!startedYet) {
